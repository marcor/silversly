...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'CartItem.desc'
        db.add_column('sales_cartitem', 'desc',
                      self.gf('django.db.models.fields.CharField')(default='desc', max_length=60),
                      keep_default=False)
        if not db.dry_run:
            items = orm['sales.CartItem'].objects.select_related('product__name').all()
            for i in items:
                i.desc = i.product.name
                i.save()


    def backwards(self, orm):
        # Deleting field 'CartItem.desc'
        db.delete_column('sales_cartitem', 'desc')
